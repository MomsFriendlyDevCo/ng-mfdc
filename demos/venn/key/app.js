		...
		controller: function($scope, $element) {
			...
			*/
			...
			$scope.$watch('venn', function() {
				...
				} else if ($scope.vennKey) {
					// Use key method {{{
					var sets = {};
					var seenKeys = {};
					// Iterate over each item and compute the initial set from data {{{
					$scope.venn.forEach(function(i) {
						if (!i[$scope.vennKey]) return; // Key not specified for this item
						$scope.getCombinations(i[$scope.vennKey]).forEach(function(combination) {
